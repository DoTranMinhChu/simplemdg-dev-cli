...
import type { TAuditLogDiscoveryCandidate, TAuditLogEnvironment } from "../api/tool-studio-api-client";
...
function EnvironmentRow({ environment, onRemoved, onResolved }: { environment: TAuditLogEnvironment; onRemoved: () => void; onResolved: () => void }): React.ReactElement {
...
export function AuditLogEnvironmentsTab({ environments, onChanged }: { environments: TAuditLogEnvironment[]; onChanged: () => void }): React.ReactElement {
  ...
  return (
    <div>
      ...
      <h2>Registered environments ({environments.length})</h2>
      {!environments.length ? (
        <EmptyState>No environments registered yet — discover some above.</EmptyState>
      ) : (
        <div className="ts-card" style={{ padding: 0 }}>
          {environments.map((environment) => (
            <EnvironmentRow key={environment.id} environment={environment} onRemoved={onChanged} onResolved={onChanged} />
          ))}
        </div>
      )}
    </div>
