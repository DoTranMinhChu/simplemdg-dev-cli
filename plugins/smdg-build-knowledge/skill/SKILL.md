...
2. Read `.claude/knowledge/flows/*` (glob existing flow directories) to see what's already been documented — each existing entry is also a candidate for a refresh, not just new discovery.
...
2. **For every flow that already has a document**: default policy — update the existing document in place, or always write a new version alongside it without touching the original? Offer both a global default and the ability to call out specific flows for the opposite treatment (e.g. "update everything except the Request flow doc, version that one instead").
...
3. **Synthesize the reports yourself** into one Markdown document following this structure — every section that applies to this flow gets written out in full; skip a section only when it's genuinely not applicable (e.g. no background jobs exist for this flow at all), never to save effort:
   ...
   No "Revision history" section — track only "Last verified" date/branch at the top; a document updated in place gets refreshed there, not appended to.

4. **Write the file** per the update-vs-version choice made in step 3:
   - **Update in place**: `.claude/knowledge/flows/<flow-slug>/<flow-slug>.md` — overwrite, refresh the "Last verified" line.
   - **New version**: glob `.claude/knowledge/flows/<flow-slug>/<flow-slug>*.md` for the highest existing version number and write `.claude/knowledge/flows/<flow-slug>/<flow-slug>-v<N+1>.md` alongside the untouched original(s) — never edit the prior version's file. Multiple versions in one flow's directory all show up as separate selectable files in Studio's viewer for that flow, so this needs no extra plumbing.
   - **New flow, no existing doc**: `.claude/knowledge/flows/<flow-slug>/<flow-slug>.md`.

## 5. Final summary

Report, for the whole run: every flow processed, its document path(s), whether it was a fresh doc/update/new-version, and every item from every flow's §12 (known gaps) pulled inline — don't make the user open every file to find what's unverified.
