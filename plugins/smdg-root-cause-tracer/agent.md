...
6. **Anchor-string grep**, scoped only to the shortlisted, available candidates — never the full multi-repo tree. Try, in order:
   ...
   d. If step 3 triggered: whether the literal topic/event string is a useful anchor **depends on which kind of repo you're grepping**. For a hub/role-service repo (e.g. `..._process_event`, `..._process_approver`, `..._config_system`, `..._background` — repos with real, repo-local messaging code), the topic string usually is logged verbatim in both the `messaging.emit(...)` call and the `messaging.on(topic, ...)` registration, making it a high-signal anchor. For a shared-engine consumer repo (an object-type's own `_process` repo, e.g. `simplemdg_srv_prd_process`) grepping for the topic string will typically find **nothing** — that repo has no messaging code of its own; it only sets `OBJECT_TYPE`/`OBJECT_SHORTNAME` env values and imports a shared workflow-engine package (e.g. `@simplemdg/helper_process`) that builds the topic at runtime as `${OBJECT_SHORTNAME}${Action}`. In that case, grep for the env values instead, and treat the shared package's own repo as the place the actual emit/on logic lives — not this domain repo.
