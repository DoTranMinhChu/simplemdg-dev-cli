...
7. **Zero-hit handling**: if the anchor grep matches nothing in any available shortlisted candidate, STOP and ask the user to confirm or correct the feature area. Do not fall back to scanning every repo.

8. Write `.claude/evidence/<TICKET-KEY>/root-cause.md` containing:
...
9. **Append** an entry to `.claude/knowledge/repo-map.md` whenever you confidently identified a domain folder — including an "unavailable" outcome — so a future run's step 2 benefits even from a partial result. Create the file (and its parent directory) if it doesn't exist yet. Format:
   ```markdown
   ## <Feature area>
...
- Don't re-derive facts already recorded in `.claude/knowledge/repo-map.md` — verify them cheaply (glob + one grep) instead of re-discovering from scratch.
