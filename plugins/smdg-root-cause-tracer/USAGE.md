...
You normally don't invoke this agent directly — it's the third step of the `smdg-jira-fix-issue` pipeline, run after `smdg-jira-reproducer` has reproduced the bug and written a `## Failure Signature` section to `reproduction-findings.md` (endpoint, exact error text, feature area, evidence paths, and optionally a `config_snapshot` citation when the lead is config/template data rather than a live API error).
...
- Writes `.claude/evidence/<TICKET-KEY>/root-cause.md` with the final classification (UI bug / Backend bug / Contract-Schema Mismatch / Data-Environment Issue / Inconclusive), the exact file:line citation, and a minimal snippet. When a `config_snapshot` lead was used, cites both the config record (file + template/field) and the `path:line` of the code that consumes it — a config-driven root cause still needs a code citation, not just the bad value.
- Appends what it learned to `.claude/knowledge/repo-map.md` so the next ticket in the same feature area skips rediscovery.
- When the symptom looks event/queue-shaped (mentions of "event", "topic", "queue", "Event Mesh", "stuck", "DLQ", etc.), also consults `.claude/knowledge/event-map.md` first to shortlist both the producer *and* consumer repo of the relevant topic, and appends what it learned back to that file — see `smdg-event-map-sweeper`'s USAGE.md for how that file gets built up. It knows the literal-topic-string anchor only works for hub/role-service repos with real messaging code — for a shared-engine `_process` repo it greps the domain's `OBJECT_SHORTNAME` env config instead, since the actual emit/on logic lives in the shared package, not the domain repo.
