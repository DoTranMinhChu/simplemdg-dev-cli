...
You normally don't invoke this agent directly — it's the third step of the `smdg-jira-fix-issue` pipeline, run after `smdg-jira-reproducer` has reproduced the bug and written a `## Failure Signature` section to `analysis.md` (endpoint, exact error text, feature area, evidence paths).
...
If invoked directly, give it the ticket key and the path to `analysis.md`. It:
