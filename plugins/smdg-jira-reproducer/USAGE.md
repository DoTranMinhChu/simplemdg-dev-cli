...
Depends on: `smdg-playwright-browsers` and `smdg-evidence-tools` (both installed automatically alongside this plugin).
...
If you already have exported evidence for the ticket — a mass-upload spreadsheet, a JSON export from
OData, screenshots, logs — hand this agent the folder path (normally passed down automatically by
`smdg-jira-fix-issue` from what you tell it up front). It reads `.json`/text files directly (Grep-filtering
a large export instead of reading the whole thing), and reads `.xlsx`/`.xlsm`/`.xltx`/`.xltm` files via
`smdg-evidence-tools`. This is useful both as a supplement to live reproduction and as the **only** input
when a ticket's own file URL isn't reachable with your account, or when the ticket references an existing
CR/Request/batch-job ID and re-running the whole batch operation again would be costly or risky —
in that case it looks for already-persisted evidence (a Monitor/Audit screen, a direct OData read) first.
