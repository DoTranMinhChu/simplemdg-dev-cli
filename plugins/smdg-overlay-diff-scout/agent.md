...
3. **Sampled content comparison**, only when the file sets match exactly. Pick 3-5 of the highest-signal files per domain — prioritize the main service handler file(s) for `_srv_` repos, the main `.cds` model file for `_db_` repos, and the primary controller/fragment for UI folders. For each sampled file, grep the core version's structural anchors (function signatures, `srv.on(` registrations, `module.exports`, top-level class/entity names) and check whether the same anchors appear in the customer's corresponding file:
