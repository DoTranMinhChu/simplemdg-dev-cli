...
A subagent that traces one named feature end-to-end — UI entry point → API handler → database entity/table → emitted event → evidence sources — and writes it up as a durable Feature Card: `.claude/knowledge/features/<feature-slug>.md`.
...
- Notes any Monitor/Audit screen, "Download Failed Items"-style export route, or OData entity exposing persisted error/evidence data it happens to notice while tracing the UI/API hops, into an `evidence_sources` field — opportunistic, not a dedicated search — so `smdg-jira-reproducer` can check a feature's Feature Card for a cheaper evidence source before a full live reproduction.
